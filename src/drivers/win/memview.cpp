...
COLORMENU hexcolormenu[] = {
		...
		{"Bookmark", PPRGB(HexBookmark)}},
					cdlcolormenu[] = {{"Code", PPRGB(CdlCode)}, {"Data", PPRGB(CdlData)}, {"PCM Data", PPRGB(CdlPcm)}, {"Code && Data", PPRGB(CdlCodeData)}, {NULL}, {"Render", PPRGB(CdlRender)}, {"Read", PPRGB(CdlRead)}, {"Render && Read", PPRGB(CdlRenderRead)}};
...
static int GetFileData(uint32 offset)
{
	...
	if (offset < 16 + PRGsize[0])
		return PRGptr[0][offset - 16];
	if (offset < 16 + PRGsize[0] + CHRsize[0])
...
{
	if (addr < 16)
		MessageBox(hMemView, "You can't edit ROM header here, however you can use NES Header Editor to edit the header if it's an iNES format file.", "Sorry", MB_OK | MB_ICONERROR);
	if ((addr >= 16) && (addr < PRGsize[0] + 16))
		*(uint8 *)(GetNesPRGPointer(addr - 16)) = data;
	if ((addr >= PRGsize[0] + 16) && (addr < CHRsize[0] + PRGsize[0] + 16))
		*(uint8 *)(GetNesCHRPointer(addr - 16 - PRGsize[0])) = data;

	return 0;
}

static int GetRomFileSize()
...
int LoadTable(const char *nameo)
{
	...
	{ /* get one line from the file */
		...
		if (charcode2 != -1)
			charcode1 = (charcode1 << 4) | charcode2;
...
void UpdateColorTable()
{
	...
	{
		if (hexBookmarks[j].editmode != EditingMode)
			continue;
...
int DeleteCheatCallB(const char *name, uint32 a, uint8 v, int compare, int s, int type, void *data)
{ // mbg merge 6/29/06 - added arg
	if (cheatwasdeleted == -1)
		return 1;
	cheatwasdeleted++;
	if (a == addrtodelete)
	{
...
void InputData(char *input)
{
	...
	if (datasize + CursorStartAddy >= MaxSize)
	{ // too big
	...
	if (datasize < 1)
		return; // avoid adjusting cursor and accidentally writing at end
	...
	void ChangeMemViewFocus(int newEditingMode, int StartOffset, int EndOffset)
	{
		...
		if (std::min(StartOffset, EndOffset) >= MaxSize)
	...
	int GetMaxSize(int EditingMode)
	{
		...
		case MODE_NES_MEMORY:
			return 0x10000;
		case MODE_NES_PPU:
			return (GameInfo->type == GIT_NSF ? 0x2000 : 0x4000);
		case MODE_NES_OAM:
			return 0x100;
		case MODE_NES_FILE:
			return 16 + CHRsize[0] + PRGsize[0]; // todo: add trainer size
		}
		return 0;
	}
	...
	LRESULT CALLBACK MemViewCallB(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
	{
		...
		case WM_INITMENUPOPUP:
			if (undo_list != 0)
				EnableMenuItem(GetMenu(hMemView), MENU_MV_EDIT_UNDO, MF_BYCOMMAND | MF_ENABLED);
			else
				EnableMenuItem(GetMenu(hMemView), MENU_MV_EDIT_UNDO, MF_BYCOMMAND | MF_GRAYED);

			if (TableFileLoaded)
				EnableMenuItem(GetMenu(hMemView), MENU_MV_FILE_UNLOAD_TBL, MF_BYCOMMAND | MF_ENABLED);
			else
				EnableMenuItem(GetMenu(hMemView), MENU_MV_FILE_UNLOAD_TBL, MF_BYCOMMAND | MF_GRAYED);
		...
		case WM_VSCROLL:
		{
			...
			if (si.nPos < si.nMin)
				si.nPos = si.nMin;
			if ((si.nPos + (int)si.nPage) > si.nMax)
				si.nPos = si.nMax - si.nPage; // mbg merge 7/18/06 added cast
			CurOffset = si.nPos * 16;
		...
		case WM_CHAR:
			if (GetKeyState(VK_CONTROL) & 0x8000)
				return 0; // prevents input when pressing ctrl+c
			char c[2];
		...
		case WM_LBUTTONUP:
			...
			{ // this reverses them if they're not right
			...
			for (int i = 0; i < POPUPNUM; i++)
			{
				if ((curAddy >= popupmenu[i].minaddress) && (curAddy <= popupmenu[i].maxaddress) && (EditingMode == popupmenu[i].editingmode))
				{
					...
					case ID_ADDRESS_FRZ_SUBMENU:
					{
						...
						int tempAddy;
						if (CursorEndAddy == -1)
							tempAddy = CursorStartAddy;
						else
							tempAddy = CursorEndAddy;																 // This is necessary because CursorEnd = -1 if only 1 address is selected
		...
		case WM_MOUSEWHEEL:
		{
			...
			if (si.nPos < si.nMin)
				si.nPos = si.nMin;
			if ((si.nPos + (int)si.nPage) > si.nMax)
				si.nPos = si.nMax - si.nPage; // added cast
			CurOffset = si.nPos * 16;
			...
			case MENU_MV_FILE_DUMP_PPU:
			{
				...
				for (i = 0; i < sizeof(bar); i++)
				{
					...
					if (i < 0x2000)
						...
						bar[i] = vnapage[(i >> 10) & 0x3][i & 0x3FF];
					else
						bar[i] = READPAL_MOTHEROFALL(i & 0x1F);
				}
				dumpToFile(bar, sizeof(bar));
				return 0;
			}
	...
	INT_PTR CALLBACK MemFindCallB(HWND hwndDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
	{
		...
		case WM_INITDIALOG:
			...
			if (FindDirectionUp)
				CheckDlgButton(hwndDlg, IDC_MEMVIEWFIND_DIR_UP, BST_CHECKED);
			else
				CheckDlgButton(hwndDlg, IDC_MEMVIEWFIND_DIR_DOWN, BST_CHECKED);

			if (FindAsText)
				CheckDlgButton(hwndDlg, IDC_MEMVIEWFIND_TYPE_TEXT, BST_CHECKED);
			else
				CheckDlgButton(hwndDlg, IDC_MEMVIEWFIND_TYPE_HEX, BST_CHECKED);

			if (FindTextBox[0])
				SetDlgItemText(hwndDlg, IDC_MEMVIEWFIND_WHAT, FindTextBox);
	...
	void FindNext()
	{
		...
		if (hMemFind)
			GetDlgItemText(hMemFind, IDC_MEMVIEWFIND_WHAT, str, 60);
		else
			strcpy(str, FindTextBox);
		...
		for (i = 0; str[i] != 0; i++)
		{
			if (!FindAsText)
			{
				...
				{
					if ((str[i] >= 'a') && (str[i] <= 'f'))
						inputc = (inputc << 4) | (str[i] - ('a' - 0xA));
					if ((str[i] >= 'A') && (str[i] <= 'F'))
						inputc = (inputc << 4) | (str[i] - ('A' - 0xA));
					if ((str[i] >= '0') && (str[i] <= '9'))
						inputc = (inputc << 4) | (str[i] - '0');
