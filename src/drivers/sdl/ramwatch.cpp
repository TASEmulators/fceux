...
static int openRamWatchEntryDialog( ramWatchWin_t *rww, std::string *name, int *addr, int *type, int *size, int idx )
{
   ...
   ew->win = gtk_dialog_new_with_buttons("RAM Watch Entry",
			GTK_WINDOW(rww->win), (GtkDialogFlags)(GTK_DIALOG_DESTROY_WITH_PARENT),
			"_Close",
			GTK_RESPONSE_OK,
			NULL);
   ...
   if ( name->size() > 0 ){
      gtk_entry_set_text( GTK_ENTRY(ew->txt_entry_name), name->c_str() );
   }
   ...
   ew->button2 = gtk_radio_button_new_with_label_from_widget ( GTK_RADIO_BUTTON (ew->button1), "2");
	gtk_box_pack_start (GTK_BOX(hbox), ew->button2, TRUE, FALSE, 1);

   ew->button4 = gtk_radio_button_new_with_label_from_widget ( GTK_RADIO_BUTTON (ew->button1), "4");
   ...
   gtk_toggle_button_set_active (GTK_TOGGLE_BUTTON (ew->button4), FALSE);
	...
	gtk_box_pack_start(GTK_BOX(gtk_dialog_get_content_area(GTK_DIALOG(ew->win))), vbox, TRUE, TRUE, 1);
...
static void  ramWatch_cell_edited_start_cb0(
...
static void  ramWatch_cell_edited_start_cb1(
      GtkCellRenderer *renderer,
      GtkCellEditable *editable,
      gchar           *path,
      ramWatchWin_t   *rww )
...
static void  ramWatch_cell_edited_start_cb2(
      GtkCellRenderer *renderer,
      GtkCellEditable *editable,
      gchar           *path,
      ramWatchWin_t   *rww )
...
static void  ramWatch_cell_edited_start_cb3(
      GtkCellRenderer *renderer,
      GtkCellEditable *editable,
      gchar           *path,
      ramWatchWin_t   *rww )
...
void closeMemoryWatchWindow(GtkWidget* w, GdkEvent* e, ramWatchWin_t *rww)
{
   ...
   for (it=ramWatchWinList.begin(); it != ramWatchWinList.end(); it++)
   {
      if ( rww == *it )
      {
         ...
         ramWatchWinList.erase(it); break;
      }
   }
...
void openMemoryWatchWindow(void)
{
   ...
   rww->ram_watch_store = gtk_tree_store_new( 4, G_TYPE_STRING, G_TYPE_STRING, G_TYPE_STRING, G_TYPE_STRING);

	rww->tree = gtk_tree_view_new_with_model(GTK_TREE_MODEL(rww->ram_watch_store));

   gtk_tree_view_set_grid_lines( GTK_TREE_VIEW(rww->tree), GTK_TREE_VIEW_GRID_LINES_VERTICAL );
   ...
   g_signal_connect(renderer, "edited", (GCallback) ramWatch_cell_edited_cb, (gpointer)rww);
   g_signal_connect(renderer, "editing-started", (GCallback) ramWatch_cell_edited_start_cb0, (gpointer)rww);
   g_signal_connect(renderer, "editing-canceled", (GCallback) ramWatch_cell_edited_cancel_cb, (gpointer)rww);
   ...
   g_object_set(renderer, "editable", TRUE, NULL);
   g_signal_connect(renderer, "edited", (GCallback) ramWatch_cell_edited_cb, (gpointer)rww);
   g_signal_connect(renderer, "editing-started", (GCallback) ramWatch_cell_edited_start_cb1, (gpointer)rww);
   g_signal_connect(renderer, "editing-canceled", (GCallback) ramWatch_cell_edited_cancel_cb, (gpointer)rww);
   ...
   g_object_set(renderer, "editable", TRUE, NULL);
   g_signal_connect(renderer, "edited", (GCallback) ramWatch_cell_edited_cb, (gpointer)rww);
   g_signal_connect(renderer, "editing-started", (GCallback) ramWatch_cell_edited_start_cb2, (gpointer)rww);
   g_signal_connect(renderer, "editing-canceled", (GCallback) ramWatch_cell_edited_cancel_cb, (gpointer)rww);
   ...
   g_object_set(renderer, "editable", TRUE, NULL);
   g_signal_connect(renderer, "edited", (GCallback) ramWatch_cell_edited_cb, (gpointer)rww);
   g_signal_connect(renderer, "editing-started", (GCallback) ramWatch_cell_edited_start_cb3, (gpointer)rww);
   g_signal_connect(renderer, "editing-canceled", (GCallback) ramWatch_cell_edited_cancel_cb, (gpointer)rww);
   ...
   button = gtk_button_new_with_label("New");
	...
	gtk_box_pack_start(GTK_BOX(gtk_dialog_get_content_area(GTK_DIALOG(rww->win))), main_vbox, TRUE, TRUE, 0);

   g_signal_connect(rww->win, "delete-event", G_CALLBACK(closeMemoryWatchWindow), rww);
	g_signal_connect(rww->win, "response", G_CALLBACK(closeMemoryWatchWindow), rww);
