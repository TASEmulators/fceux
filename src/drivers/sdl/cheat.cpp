                              ...
                              cheat_win_t *cw)
{
	...
	FCEUI_CheatSearchEnd(FCEU_SEARCH_NEWVAL_KNOWN, cw->cheat_search_known_value, 0);
	cw->showCheatSearchResults();
}
...
static void cheatSearchNotEqual( GtkButton *button,
                                 cheat_win_t *cw )
{
	int checked = gtk_toggle_button_get_active( GTK_TOGGLE_BUTTON(cw->neq_chkbox) );

	//printf("Cheat Search NotEqual %i!\n", checked);

	if ( checked ){
	   FCEUI_CheatSearchEnd(FCEU_SEARCH_PUERLY_RELATIVE_CHANGE, 0, cw->cheat_search_neq_value);
	} else {
...
static void cheatSearchGreaterThan( GtkButton *button,
                                    cheat_win_t *cw )
{
	int checked = gtk_toggle_button_get_active( GTK_TOGGLE_BUTTON(cw->gt_chkbox) );

	//printf("Cheat Search GreaterThan %i!\n", checked);

	if ( checked ){
	   FCEUI_CheatSearchEnd(FCEU_SEARCH_NEWVAL_GT_KNOWN, 0, cw->cheat_search_gt_value);
	} else {
...
static void cheatSearchLessThan( GtkButton *button,
                                 cheat_win_t *cw )
{
	int checked = gtk_toggle_button_get_active( GTK_TOGGLE_BUTTON(cw->lt_chkbox) );

	//printf("Cheat Search LessThan %i!\n", checked);

	if ( checked ){
	   FCEUI_CheatSearchEnd(FCEU_SEARCH_NEWVAL_LT_KNOWN, 0, cw->cheat_search_lt_value);
	} else {
...
static void cheatSearchValueEntryCB1( GtkWidget *widget,
                                     cheat_win_t *cw )
{
	long value;
...
static void cheatSearchValueEntryCB2( GtkWidget *widget,
                                     cheat_win_t *cw )
{
	long value;
	const gchar *entry_text;
...
static void cheatSearchValueEntryCB3( GtkWidget *widget,
                                     cheat_win_t *cw )
{
	long value;
	const gchar *entry_text;
...
static void cheatSearchValueEntryCB4( GtkWidget *widget,
                                     cheat_win_t *cw )
{
	long value;
	const gchar *entry_text;
...
static int activeCheatListCB(char *name, uint32 a, uint8 v, int c, int s, int type, void* data)
{
   ...
   if ( !cw->actv_cheat_redraw )
   {
      if ( !gtk_tree_model_iter_next( GTK_TREE_MODEL(cw->actv_cheats_store), &cw->actv_cheats_iter ) )
      {
	      gtk_tree_store_append( cw->actv_cheats_store, &cw->actv_cheats_iter, NULL); // aquire iter
      }
   }
...
static void newCheatEntryCB1( GtkWidget *widget,
                             cheat_win_t *cw )
{
	const gchar *entry_text;
   entry_text = gtk_entry_get_text (GTK_ENTRY (widget));
...
static void newCheatEntryCB2( GtkWidget *widget,
                             cheat_win_t *cw )
{
	const gchar *entry_text;
   entry_text = gtk_entry_get_text (GTK_ENTRY (widget));
...
static void newCheatEntryCB3( GtkWidget *widget,
                             cheat_win_t *cw )
{
	const gchar *entry_text;
   entry_text = gtk_entry_get_text (GTK_ENTRY (widget));
...
static void newCheatEntryCB4( GtkWidget *widget,
                             cheat_win_t *cw )
{
	const gchar *entry_text;
   entry_text = gtk_entry_get_text (GTK_ENTRY (widget));
                             ...
                             cheat_win_t *cw )
{
   ...
   {
		if ( FCEUI_AddCheat( cw->new_cheat_name.c_str(), cw->new_cheat_addr, cw->new_cheat_val, cw->new_cheat_cmp, 1) )
		{
		   updateAllActvCheatLists(1);
		}
	}
                             ...
                             cheat_win_t *cw )
{
	...
	treeSel = gtk_tree_view_get_selection( GTK_TREE_VIEW(cw->actv_cheat_tree) );
...
void openCheatsWindow(void)
{
   ...
   cheatWinList.push_back(cw);
	...
	cw->actv_cheat_tree = gtk_tree_view_new_with_model(GTK_TREE_MODEL(cw->actv_cheats_store));

	GtkCellRenderer *renderer;
	GtkCellRenderer *chkbox_renderer;
	GtkTreeViewColumn* column;
	
   gtk_tree_view_set_grid_lines( GTK_TREE_VIEW(cw->actv_cheat_tree), GTK_TREE_VIEW_GRID_LINES_VERTICAL );
	...
	gtk_tree_view_append_column(GTK_TREE_VIEW(cw->actv_cheat_tree), column);

	renderer = gtk_cell_renderer_text_new();
   g_object_set(renderer, "family", "MonoSpace", NULL);
   g_object_set(renderer, "editable", TRUE, NULL);
   g_signal_connect(renderer, "edited", (GCallback) cheat_cell_edited_cb1, (gpointer)cw);
   ...
   g_object_set(renderer, "family", "MonoSpace", NULL);
   g_object_set(renderer, "editable", TRUE, NULL);
   g_signal_connect(renderer, "edited", (GCallback) cheat_cell_edited_cb2, (gpointer)cw);
   ...
   g_object_set(renderer, "family", "MonoSpace", NULL);
   g_object_set(renderer, "editable", TRUE, NULL);
   g_signal_connect(renderer, "edited", (GCallback) cheat_cell_edited_cb3, (gpointer)cw);
	column = gtk_tree_view_column_new_with_attributes("Cmp", renderer, "text", 3, NULL);
	gtk_tree_view_append_column(GTK_TREE_VIEW(cw->actv_cheat_tree), column);

	renderer = gtk_cell_renderer_text_new();
   g_object_set(renderer, "editable", TRUE, NULL);
   g_signal_connect(renderer, "edited", (GCallback) cheat_cell_edited_cb4, (gpointer)cw);
   ...
   g_signal_connect( txt_entry, "changed",
	...
	g_signal_connect( button, "clicked",
		      G_CALLBACK (pauseDuringCheatWinActvCB), (gpointer) cw );
   ...
   g_signal_connect( txt_entry, "changed",
	...
	cw->ram_match_store = gtk_tree_store_new( 3, G_TYPE_STRING, G_TYPE_STRING, G_TYPE_STRING);

	cw->search_cheat_tree = gtk_tree_view_new_with_model(GTK_TREE_MODEL(cw->ram_match_store));

   gtk_tree_view_set_grid_lines( GTK_TREE_VIEW(cw->search_cheat_tree), GTK_TREE_VIEW_GRID_LINES_VERTICAL );

	renderer = gtk_cell_renderer_text_new();
   g_object_set(renderer, "family", "MonoSpace", NULL);
	column = gtk_tree_view_column_new_with_attributes("Addr", renderer, "text", 0, NULL);
	gtk_tree_view_append_column(GTK_TREE_VIEW(cw->search_cheat_tree), column);
	...
	g_signal_connect(win, "delete-event", G_CALLBACK(closeCheatDialog), cw);
