 ...
 */
 ...
 // mapper 3873 - Rainbow2 board v1.0 revA by Broke Studio
 //
...
static void esp_check_new_message() {
	...
	if (esp_enable && esp->getDataReadyIO() && has_esp_message_received == false)
	{
		...
		FPGA_RAM[rx_address << 8] = message_length;
		for (uint8 i = 0; i < message_length; i++)
		{
			FPGA_RAM[(rx_address << 8) + 1 + i] = esp->tx();
		}
		rx_index = 0;
...
static void Sync(void) {
	...
	// 32K
	if (prg_rom_mode == PRG_ROM_MODE_0)
	{
		// PRG-ROM
		if (!(prg[3] & 0x8000))
	...
	// 16K + 16K
	if (prg_rom_mode == PRG_ROM_MODE_1)
	{
		// PRG-ROM
		if (!(prg[3] & 0x8000))
	...
	// 16K + 8K + 8K
	if (prg_rom_mode == PRG_ROM_MODE_2)
	{
		// PRG-ROM
		if (!(prg[3] & 0x8000))
			setprg16r(0x11, 0x8000, prg[3] & 0x7fff);
	...
	// 8K + 8K + 8K + 8K
	if (prg_rom_mode == PRG_ROM_MODE_3)
	{
		for (uint8 i = 0; i < 4; i++)
		{
			// PRG-ROM
	...
	for (int i = 0; i < 4; i++)
	{
		...
		case NT_CIRAM:
			...
			break;
		case NT_CHR_RAM:
			vnapage[i] = CHRRAM + 0x400 * cur_NT_bank;
			break;
		case NT_FPGA_RAM:
...
static DECLFW(WRAM_0x6000Wr)
{
	...
	if (prg_ram_mode == PRG_RAM_MODE_0)
	{
		if (((prg[1] & 0xC000) >> 14) == 0x11)
			FPGA_RAM[A & 0x1fff] = V; // FPGA-RAM
		else
			CartBW(A, V); // PRG-ROM / WRAM
	}
...
static DECLFR(WRAM_0x6000Rd)
{
	...
	{
		if (((prg[1] & 0xC000) >> 14) == 0x11)
			return FPGA_RAM[A & 0x1fff]; // FPGA-RAM
		else
			return CartBR(A); // PRG-ROM / WRAM
	}
...
{
	FPGA_RAM[(A & 0xfff) | ((prg[0] & 0x01) << 12)] = V;
}
static DECLFR(WRAM_0x5000Rd)
{
	return FPGA_RAM[(A & 0xfff) | ((prg[0] & 0x01) << 12)];
	return 0x00;
}
...
static DECLFR(Rainbow2Read) {
	...
	case 0x4162: return (mul_result >> 8) & 0xff;
...
static DECLFW(Rainbow2Write) {
	...
	case 0x4151: S_IRQcontrol |= (V & 0x01); break;
	...
	case 0x415B:
		...
		break;
		// Multiplier
	case 0x4161: mul_a = V; mul_result = mul_a * mul_b; break;
	case 0x4162: mul_b = V; mul_result = mul_a * mul_b; break;
		// ESP - WiFi
	case 0x4170:
		esp_enable = V & 0x01;
...
void RAINBOW2_Init(CartInfo *info) {
	...
	if (info->battery)
	{
		info->SaveGame[save_game_index] = FPGA_RAM;
		info->SaveGameLen[save_game_index] = FPGA_RAMSIZE;
		save_game_index++;
	}
